...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'Registration', fields ['user']
        db.delete_unique('courses_registration', ['user_id'])

        # Removing unique constraint on 'Registration', fields ['group']
        db.delete_unique('courses_registration', ['group_id'])

        # Adding unique constraint on 'Registration', fields ['group', 'user']
        db.create_unique('courses_registration', ['group_id', 'user_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Registration', fields ['group', 'user']
        db.delete_unique('courses_registration', ['group_id', 'user_id'])

        # Adding unique constraint on 'Registration', fields ['group']
        db.create_unique('courses_registration', ['group_id'])

        # Adding unique constraint on 'Registration', fields ['user']
        db.create_unique('courses_registration', ['user_id'])
    ...
    models = {
        ...
        'courses.registration': {
            'Meta': {'unique_together': "[('user', 'group')]", 'object_name': 'Registration'},
            'group': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['courses.Group']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']"})
        },
